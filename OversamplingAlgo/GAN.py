...
def binary_crossentropy(preds, targets, offset=1e-10, name=None):
    ...
    with ops.op_scope([preds, targets], name, 'bce_loss') as name:
        ...
        return tf.reduce_mean(-(targets * tf.log(preds + offset) +
...
class GAN(object):
    ...
    def __init__(self, num_epochs, batch_size, d_hidden_dim, g_hidden_dim,
                 n_input, stddev=0.5, pretrain=False, d_transfer_fct=lrelu,
                 g_transfer_fct=tf.nn.relu, W_init_fct=init_xavier,
                 b_init_fct=tf.zeros, d_learning_rate=0.01,
                 g_learning_rate=0.0005, random_state=None, log_every=None):
    ...
    def _create_networks(self):
        ...
        if self.d_pretrain:
            with tf.variable_scope('D_pre'):
                self.pre_input = tf.placeholder(tf.float32, shape=(
    ...
    def partial_fit(self, X, Z):
        ...
        opt_d, cost_d = self.sess.run((self.opt_d, self.loss_d),
                                      feed_dict={self.z: Z, self.x: X})
        # Update generator.
        opt_g, cost_g = self.sess.run((self.opt_g, self.loss_g),
                                      feed_dict={self.z: Z})
